...
from gerenciador.models import Character, DeDCharacterSpecs, OrdemCharacterSpecs, Option, Race, RPGClass, System, Tips
...
class CharacterViewSet(viewsets.ViewSet):
    ...
    def create(self, request):
        ...
        try:
            ...
            stats = self._create_system_specs(
                system,
                character_base,
                score,
                specs_enviadas,
            ...
            return Response(
                {
                    ...
                    "system": system.name,
                    "suggested_class": suggested_class.name if suggested_class else "Aventureiro",
                    "classSugerida": suggested_class.name if suggested_class else "Aventureiro",
                    "profile": profile_name,
                    "background": profile_name,
                    "roleplay_tip": matching_tip.roleplay_text if matching_tip else "Jogue livremente!",
                    "tips": [matching_tip.roleplay_text if matching_tip else "Jogue livremente!"],
                    "quote": "A aventura comeca agora.",
                    "stats": stats,
                    "pvs": character_base.pvs,
                    "armor_class": character_base.armor_class,
                },
    ...
    def _get_matching_tip(self, system, target_profile):
        ...
        if matching_tip:
            rpg_class = RPGClass.objects.filter(
                system=system,
                name__iexact=matching_tip.suggested_class,
            ).first()
            return matching_tip, rpg_class, matching_tip.get_target_profile_display()

        return None, None, "Equilibrado"
    ...
    def _create_system_specs(self, system, character_base, score, specs_enviadas, chosen_race_id):
        ...
        except (Race.DoesNotExist, TypeError):
            race = None
        ...
        if system_name in ["d&d", "dnd", "d&d 5e", "trpg", "tormenta", "tormenta rpg", "tormenta 20", "t20"]:
            ...
            DeDCharacterSpecs.objects.create(
                ...
                wisdom=wis_final,
                charisma=cha_final,
                mana_points=calculated_pms or specs_enviadas.get("mana_points", 0),
            )
            stats = {
                "strength": str_final,
                "dextery": dex_final,
            ...
            agi_final = score["DEX"] + 1 + (race.dex_bonus if race else 0)
            vig_final = score["CON"] + 1 + (race.con_bonus if race else 0)
            int_final = score["INT"] + 1 + (race.int_bonus if race else 0)
