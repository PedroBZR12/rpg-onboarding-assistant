...
const CONFIG = {
    usarMock: false,
    systemsUrl: '/api/systems/',
    quizBySystemUrl: '/api/quiz/by-system/',
...
class DjangoApiService {
    static async getSystems() {
        if (state.systems.length) return state.systems;

        const systems = await this._fetchJson(CONFIG.systemsUrl);
        state.systems = systems;
        return systems;
    }

    static async getSystemByName(systemName) {
        const systems = await this.getSystems();
        const normalizedName = this._normalize(systemName);

        return systems.find(system => this._normalize(system.name) === normalizedName)
            || systems.find(system => normalizedName.includes(this._normalize(system.name)))
            || systems.find(system => this._normalize(system.name).includes(normalizedName));
    }

    static async getQuestions(systemId) {
        const questions = await this._fetchJson(`${CONFIG.quizBySystemUrl}${systemId}/`);
        return questions.map(question => ({
            ...
            options: question.options.map(option => ({
                id: option.id,
                text: option.text,
                value: option.target_attribute,
                trait: option.target_attribute
            }))
        }));
    }

    static async createCharacter(system, answers) {
        const payload = {
            system_id: system.id,
            chosen_options: answers.map(answer => answer.id),
            name: 'Aventureiro da Jornada',
            movement: 9,
            size: 'Medio',
            level: 1
        };

        const character = await this._fetchJson(CONFIG.charactersUrl, {
            method: 'POST',
            headers: this._jsonHeaders(),
            body: JSON.stringify(payload)
        });

        return this._normalizeCharacterSheet(system, answers, character);
    }

    static _normalizeCharacterSheet(system, answers, character) {
        return {
            name: character.name || 'Aventureiro da Jornada',
            system: system.name,
            stats: character.stats || this._statsFromAnswers(system.name, answers),
            classSugerida: character.classSugerida || character.suggested_class || 'Aventureiro',
            background: character.background || character.profile || 'Perfil criado a partir das escolhas do quiz.',
            tips: character.tips || [character.roleplay_tip || 'Jogue livremente e siga as escolhas do seu personagem.'],
            quote: character.quote || 'A aventura comeca agora.',
            traits: answers.map(answer => answer.trait || answer.value)
        };
    }

    static _statsFromAnswers(systemName, answers) {
        const isOrdem = systemName === 'Ordem Paranormal';
        const stats = isOrdem
            ? { strength: 1, agility: 1, constitution: 1, intelligence: 1, presence: 1 }
            : { strength: 10, dextery: 10, constitution: 10, intelligence: 10, wisdom: 10, charisma: 10 };

        const statMap = isOrdem
            ? { STR: 'strength', DEX: 'agility', CON: 'constitution', INT: 'intelligence', WIS: 'intelligence', CHA: 'presence' }
            : { STR: 'strength', DEX: 'dextery', CON: 'constitution', INT: 'intelligence', WIS: 'wisdom', CHA: 'charisma' };

        answers.forEach(answer => {
            const statName = statMap[answer.value];
...
class AppController {
    ...
    static async selectSystem(systemName) {
        ...
        try {
            ...
            if (CONFIG.usarMock) {
                questions = systemName === 'Ordem Paranormal' ? ordemParanormalQuestions : dndAndTormentaQuestions;
            } else {
                const system = await DjangoApiService.getSystemByName(systemName);

                if (!system) {
                    throw new Error(`Sistema "${systemName}" nao encontrado na API.`);
                }

                state.selectedSystem = system.name;
                state.selectedSystemId = system.id;
                questions = await DjangoApiService.getQuestions(system.id);
            }

            if (!questions.length) {
                throw new Error('Nenhuma pergunta encontrada para este sistema.');
            }

            state.questions = questions;
            this.navigateTo('quiz');
            QuizView.render(state.selectedSystem, state.currentQuestionIndex, state.questions, (option) => this.handleOptionSelection(option));
        } catch (error) {
            ToastView.show(error.message || 'Erro ao carregar perguntas.');
    ...
    static handleOptionSelection(option) {
        ...
        const questions = state.questions.length
            ? state.questions
            : (state.selectedSystem === 'Ordem Paranormal' ? ordemParanormalQuestions : dndAndTormentaQuestions);
    ...
    static async executeCharacterGeneration() {
        ...
        try {
            ...
            if (CONFIG.usarMock) {
                ...
                const system = state.systems.find(item => item.id === state.selectedSystemId)
                    || await DjangoApiService.getSystemByName(state.selectedSystem);
                characterSheet = await DjangoApiService.createCharacter(system, state.answers);
            }
            ...
            clearInterval(interval);
            ToastView.show(error.message || "Erro ao obter dados do servidor.");
            this.navigateTo('home');
        }
    }
    ...
    static async saveStats() {
        ...
        try {
            if (CONFIG.usarMock) {
                ...
                const narrative = MockApiService._calculateNarrative(state.selectedSystem, state.tempStats);
                updatedCharacter = {
                    ...state.calculatedCharacter,
